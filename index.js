    ...
    if(message.channel.type != 'dm' && message.channel.type != 'groupdm'){
        if((message.content.indexOf(prefix) == 0)){
            ...
            if(command === "colour" || command === "color"){
                ...
                var phatpos = parseInt(message.guild.roles.filter(f => f.name == "phat" && f.color == '#000001').last().position)+1;
                    ...
                    },
        {
            ...
            if(command === "8ball"){
               ...
               if(!args.join(' ').endsWith("?")){
                ...
                if(ques.split('').length > 66) return message.reply(`would you please try asking a question that isn't ${ques.split('').length} characters long`);
               message.channel.sendEmbed({
                   color: 0xffe23c,
                   author: {
                       name: `▶${ques}◀`,
            ...
            if(command === "ship"){
                ...
                if(thing1.split('').length > 50 || thing2.split('').length > 50) return message.reply("OI MATE WOULD YOU MIND PICKING A SHORTER NAME");
